...
dataframe = pd.read_csv("log/0.3helpdesk.csv", low_memory=False, index_col=0)
...
class CNN_Net1(nn.Module):
    def __init__(self):
        ...
        self.out_conv = nn.Conv1d(64, 33, kernel_size=1)
...
def trainandtest(train_loader,test_loader,cnn,loss_func,lrc,train_dataset,epochs,device):
    ...
    torch.save(model, 'train_helpdesk_t1' + str(lrc) + model.getstr())
    model.eval()
    testloss = 0
    testacc = 0
    outlist = []
...
if __name__ == '__main__':
    ...
    # print(acclist31)
    pd1 = loadmodelandpre(test_loader1, 'train_helpdesk_t10.001CNN_Net1', device)
    pd2 = loadmodelandpre(test_loader2, 'train_helpdesk_t10.001CNN_Net2', device)
    pd3 = loadmodelandpre(test_loader3, 'train_helpdesk_t10.001CNN_Net3', device)
    print(pd1)
    print(pd2)
    print(pd3)
    # print(prelist[:10])
