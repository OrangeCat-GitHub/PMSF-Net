...
dataframe = pd.read_csv("log/0.3road.csv", low_memory=False, index_col=0)
...
class CrossAttentionFusion(nn.Module):
    ...
    def forward(self, Q_feature, K_feature):
        ...
        # print(f"Q_feature.shape = {Q_feature.shape}")
        # print(f"K_feature.shape = {K_feature.shape}")
...
class CNN_Net1(nn.Module):
    def __init__(self):
        ...
        self.conv1 = Pinwheel_shaped_Convolution(c1=6, c2=8, k=3, s=1)
        self.conv2 = nn.Conv2d(8, 32, kernel_size=3, padding=1)  # 修改1：减小kernel，加padding
        self.conv2_drop = nn.Dropout2d()
        self.conv3 = nn.Conv2d(32, 64, kernel_size=3, padding=1)

        self.msf = MSF(feature_dim=64)
        self.out_conv = nn.Conv1d(64, 11, kernel_size=1)

    def forward(self, x):
        B, C, H, W = x.shape
        assert H >= 8 and W >= 8, f"Input too small: got H={H}, W={W}, require at least 8x8."

        # print(f"[Input] x: {x.shape}")
        low = F.relu(F.max_pool2d(self.conv1(x), 2))
        # print(f"[After conv1 + pool] low: {low.shape}")

        mid = F.relu(self.conv2_drop(self.conv2(low)))  # 修改2：去掉第二个池化
        # print(f"[After conv2 + dropout] mid: {mid.shape}")

        if mid.shape[-1] < 2 or mid.shape[-2] < 2:
            raise ValueError(f"Feature map too small before third pooling: {mid.shape}")

        high = F.relu(F.max_pool2d(self.conv3(mid), 2))
        # print(f"[After conv3 + pool] high: {high.shape}")

        # Resize low/mid to high.shape for fusion
        ...
        # Pad channels to 64
        low_flat = F.pad(low_flat, (0, 0, 0, 0, 0, 64 - low_flat.shape[1]))
        mid_flat = F.pad(mid_flat, (0, 0, 0, 0, 0, 64 - mid_flat.shape[1]))

        # Flatten spatial dims for attention fusion
...
class CNN_Net2(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(8, 32, kernel_size=3, padding=1)  # 修改1
        self.conv2_drop = nn.Dropout2d()
        self.conv3 = nn.Conv2d(32, 64, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        low = F.relu(F.max_pool2d(self.conv1(x), 2))  # [B, 8, H/2, W/2]
        mid = F.relu(self.conv2_drop(self.conv2(low)))  # [B, 32, H/2, W/2]  # 修改2：去掉pool
        high = F.relu(F.max_pool2d(self.conv3(mid), 2))  # [B, 64, H/4, W/4]
...
class CNN_Net3(nn.Module):
    ...
    def forward(self, x):
        # First conv + pool
        low = F.relu(F.max_pool2d(self.conv1(x), 2))   # ↓ 1/2

        # Second conv + dropout + pool
        mid = F.relu(F.max_pool2d(self.conv2_drop(self.conv2(low)), 2))  # ↓ 1/2 again

        # Third conv
        high_conv = self.conv3(mid)

        # Conditionally apply pooling to avoid dimension becoming zero
        if high_conv.shape[-1] >= 2 and high_conv.shape[-2] >= 2:
            high = F.relu(F.max_pool2d(high_conv, 2))
        else:
            high = F.relu(high_conv)

        # Upsample low/mid to match high
        low_flat = F.interpolate(low, size=high.shape[2:], mode='bilinear', align_corners=False)
        mid_flat = F.interpolate(mid, size=high.shape[2:], mode='bilinear', align_corners=False)

        # Zero pad channel dimension to 64
        low_flat = F.pad(low_flat, (0, 0, 0, 0, 0, 64 - low_flat.size(1)))
        mid_flat = F.pad(mid_flat, (0, 0, 0, 0, 0, 64 - mid_flat.size(1)))

        # Flatten spatial to seq
        low_flat = low_flat.view(low_flat.size(0), 64, -1)
        mid_flat = mid_flat.view(mid_flat.size(0), 64, -1)
        high_flat = high.view(high.size(0), 64, -1)
...
def trainandtest(train_loader,test_loader,cnn,loss_func,lrc,train_dataset,epochs,device,ix):
    ...
    for epoch in range(epochs):
        ...
        if epoch==10 and 100 * running_acc<98:
            break
    torch.save(model, 'train_roadfine_t1' + str(lrc) + model.getstr())
    model.eval()
    testloss = 0
    testacc = 0
    outlist = []
...
if __name__ == '__main__':
    ...
    start_time = time()
    # for i in range(30):
    # acclist31.append(trainandtest(train_loader3, test_loader3, CNN_Net3, loss_func, 0.001, train_dataset1, 50,device,i))
    ...
    # print(acclist31)
    pd1 = loadmodelandpre(test_loader1, 'train_roadfine_t30.001CNN_Net1', device)
    pd2 = loadmodelandpre(test_loader2, 'train_roadfine_t30.001CNN_Net2', device)
    pd3 = loadmodelandpre(test_loader3, 'train_roadfine_t30.001CNN_Net3', device)
    print(pd1)
    print(pd2)
    print(pd3)
    # print(prelist[:10])
    print("MAX——————————————————————————————————")
    print(max(pd1, pd2, pd3))
    print("MAX——————————————————————————————————")
    end_time=time()
