...
dataframe = pd.read_csv("log/0.15hospital.csv", low_memory=False, index_col=0)
...
class CNN_Net1(nn.Module):
    def __init__(self):
        ...
        self.pconv = Pinwheel_shaped_Convolution(c1=6, c2=8, k=3, s=1)
        self.conv2 = nn.Conv2d(8, 32, kernel_size=7)
        self.conv2_drop = nn.Dropout2d()

        # 特征融合后的池化，压缩空间维度
        self.pool = nn.AdaptiveAvgPool2d((1, 100))  # 输出大小 [B, C, 100]
        self.msf = MSF(feature_dim=32)

        # 融合后进行分类
        self.fc_out = nn.Linear(32, 17)

    def forward(self, x):
        x = F.relu(self.pconv(x))  # PConv替换conv1
        low = F.relu(F.max_pool2d(self.conv2_drop(self.conv2(x)), 1))  # conv2保持不变

        # 为构造3层特征，复制中间层并 squeeze 掉 1 维
        low_ = self.pool(low).squeeze(2)  # [B, C, 100]
        mid_ = self.pool(low).squeeze(2)
        high_ = self.pool(low).squeeze(2)

        fused = self.msf(low_, mid_, high_)  # [B, C, 100]
        fused = fused.mean(dim=2)  # 全局平均池化，得到 [B, C]
...
class CNN_Net2(nn.Module):
    def __init__(self):
        ...
        self.pconv = Pinwheel_shaped_Convolution(c1=1, c2=8, k=3, s=1)
        self.conv2 = nn.Conv2d(8, 32, kernel_size=7)
        self.conv2_drop = nn.Dropout2d()

        self.pool = nn.AdaptiveAvgPool2d((1, 100))  # 输出 [B, C, 100]
        self.msf = MSF(feature_dim=32)
        self.fc_out = nn.Linear(32, 17)

    def forward(self, x):
        x = F.relu(self.pconv(x))
        low = F.relu(F.max_pool2d(self.conv2_drop(self.conv2(x)), 1))

        low_ = self.pool(low).squeeze(2)
        mid_ = self.pool(low).squeeze(2)
        high_ = self.pool(low).squeeze(2)

        fused = self.msf(low_, mid_, high_)
...
class CNN_Net3(nn.Module):
    def __init__(self):
        ...
        self.pconv = Pinwheel_shaped_Convolution(c1=3, c2=8, k=3, s=1)
        self.conv2 = nn.Conv2d(8, 32, kernel_size=7)
        self.conv2_drop = nn.Dropout2d()
...
def trainandtest(train_loader,test_loader,cnn,loss_func,lrc,train_dataset,epochs,device):
    ...
    torch.save(model, 'train_hospital_t1' + str(lrc) + model.getstr())
    model.eval()
    testloss = 0
    testacc = 0
    outlist = []
...
if __name__ == '__main__':
    ...
    # print(acclist32)
    # print(acclist33)
    # pd1, prelist = loadmodelandpre(test_loader1, 'train_hospital_t10.003CNN_Net1', device)
    # pd2, _ = loadmodelandpre(test_loader2, 'train_hospital_t10.003CNN_Net2', device)
    # pd3, _ = loadmodelandpre(test_loader3, 'train_hospital_t10.003CNN_Net3', device)
    pd1 = loadmodelandpre(test_loader1, 'train_hospital_t10.003CNN_Net1', device)
    pd2 = loadmodelandpre(test_loader2, 'train_hospital_t10.003CNN_Net2', device)
    pd3 = loadmodelandpre(test_loader3, 'train_hospital_t10.003CNN_Net3', device)
    print(pd1)
    print(pd2)
    print(pd3)
    # print(prelist[:10])
