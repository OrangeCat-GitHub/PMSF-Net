...
#         x = F.relu(self.fc1(x))
#         x = F.relu(self.fc2(x))
#         # print("3.查看通道数查看通道数查看通道数查看通道数查看通道数查看通道数查看通道数查看通道数")
#         # print(x.shape)
#         return F.log_softmax(x, dim=1)
...
def trainandtest(train_loader,test_loader,cnn,loss_func,lrc,train_dataset,epochs,device):
    ...
    torch.save(model, 'train_bpi2012_t1' + str(lrc) + model.getstr())
    model.eval()
    testloss = 0
    testacc = 0
    outlist = []
...
if __name__ == '__main__':
    ...
    acclist31.append(trainandtest(train_loader1, test_loader1, CNN_Net1, loss_func, 0.003, train_dataset2, 100, device))
    acclist31.append(trainandtest(train_loader2, test_loader2, CNN_Net2, loss_func, 0.003, train_dataset3, 100, device))
    acclist31.append(trainandtest(train_loader3, test_loader3, CNN_Net3, loss_func, 0.003, train_dataset1, 100, device))
    print(acclist31)
    end_time=time()
