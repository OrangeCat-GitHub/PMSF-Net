...
dataframe = pd.read_csv("log/0.3bpi2013.csv", low_memory=False, index_col=0)
...
class CNN_Net1(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(8, 32, kernel_size=(3, 2), padding=1)
    ...
    def forward(self, x):
        ...
        # print(f"low shape: {low.shape}")  # Debug
...
#         self.input_reduce = nn.Conv2d(8, 1, kernel_size=1)  # 降维到1通道
...
class CNN_Net2(nn.Module):
    def __init__(self):
        ...
        self.input_reduce = nn.Conv2d(8, 1, kernel_size=1)  # 降维到1通道
        self.conv1 = Pinwheel_shaped_Convolution(c1=1, c2=8, k=3, s=1)
        self.conv2 = nn.Conv2d(8, 32, kernel_size=3, padding=1)  # 修改 kernel_size
        self.conv2_drop = nn.Dropout2d()
        self.conv3 = nn.Conv2d(32, 64, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        low = F.relu(F.max_pool2d(self.conv1(x), 2))
        mid = F.relu(F.max_pool2d(self.conv2_drop(self.conv2(low)), 2))
        high = F.relu(F.adaptive_max_pool2d(self.conv3(mid), output_size=(2, 2)))  # 修改池化策略
...
class CNN_Net3(nn.Module):
    def __init__(self):
        ...
        self.input_reduce = nn.Conv2d(8, 3, kernel_size=1)  # 降维到3通道
        self.conv1 = Pinwheel_shaped_Convolution(c1=3, c2=8, k=3, s=1)
        self.conv2 = nn.Conv2d(8, 32, kernel_size=4, stride=1, padding=1)  # 增加padding来避免尺寸过小
        self.conv2_drop = nn.Dropout2d()
        self.conv3 = nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1)  # 增加padding

        self.msf = MSF(feature_dim=64)
        self.out_conv = nn.Conv1d(64, 36, kernel_size=1)

    def forward(self, x):
        x = self.input_reduce(x)  # 把8通道变成3通道
        low = F.relu(F.max_pool2d(self.conv1(x), 2))
        mid = F.relu(F.max_pool2d(self.conv2_drop(self.conv2(low)), 2))
        high = F.relu(F.adaptive_max_pool2d(self.conv3(mid), output_size=(4, 1)))  # 设定合理输出尺寸
...
#     torch.save(model, 'train_bpi2013_t1' + str(lrc) + model.getstr())
#     model.eval()
#     testloss = 0
#     testacc = 0
#     outlist = []
...
def trainandtest(train_loader, test_loader, cnn, loss_func, lrc, train_dataset, epochs, device):
    ...
    for epoch in range(epochs):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            # 释放显存
            del batch_x, batch_y, out, loss, predict
            torch.cuda.empty_cache()

        running_loss /= len(train_dataset)
        running_acc /= len(train_dataset)
        print(f"[{epoch + 1}/{epochs}] Loss: {running_loss:.5f}, Acc: {running_acc * 100:.2f}%")

    # 保存模型
    torch.save(model.state_dict(), f'train_bpi2013_t1_{lrc}.pth')
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            # 确保损失函数匹配模型输出
            testloss += F.cross_entropy(output, target, reduction='sum').item()

            pred = output.max(1)[1]  # 预测的类别
            testacc += pred.eq(target).sum().item()

    testloss /= len(test_loader.dataset)
    test_accuracy = 100. * testacc / len(test_loader.dataset)

    print(
        f"\nTest set: Average loss: {testloss:.4f}, Accuracy: {testacc}/{len(test_loader.dataset)} ({test_accuracy:.2f}%)\n")

    # 释放显存
    del model, opt, pred, output, data, target
    torch.cuda.empty_cache()
    gc.collect()
...
def loadmodelandpre(test_loader, smodel, device):
    # 直接加载完整模型（而非state_dict）
    model = torch.load(smodel, map_location=device, weights_only=False)  # 解决FutureWarning
    model.eval()  # 设置为推理模式
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            # 计算损失（匹配模型输出的log_softmax，使用nll_loss）
            testloss += F.nll_loss(output, target, reduction='sum').item()
            pred = output.max(1)[1]  # 获取预测类别
    ...
    from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score
    testloss /= len(test_loader.dataset)
    accuracy = accuracy_score(target_list, pred_list) * 100
    precision = precision_score(target_list, pred_list, average='macro', zero_division=0) * 100
    recall = recall_score(target_list, pred_list, average='macro', zero_division=0) * 100
    f1 = f1_score(target_list, pred_list, average='macro', zero_division=0) * 100

    # 打印指标
    print(f"\n推理结果:")
    print(f"平均损失: {testloss:.4f}")
    print(f"成功率: {accuracy:.2f}%")  # 此处成功率与准确率一致，可根据需求修改
    ...
    # 释放资源
    del model, data, target, output, pred
    torch.cuda.empty_cache()
    gc.collect()

    return accuracy, precision, recall, f1, pred_list, target_list
...
if __name__ == '__main__':
    ...
    # # print(acclist31)
    # pd1, prelist = loadmodelandpre(test_loader1, 'train_bpi2013_t10.003CNN_Net3', device)
    # # pd2, _ = loadmodelandpre(test_loader2, 'train_bpi2013_t10.003CNN_Net2', device)
    # # pd3, _ = loadmodelandpre(test_loader3, 'train_bpi2013_t10.003CNN_Net1', device)
    # print(pd1)
    # # print(pd2)
    # # print(pd3)
    # print(prelist[:10])
    ...
    start_time = time()
    # 加载模型并推理
    acc, prec, rec, f1, prelist, targetlist = loadmodelandpre(
        test_loader1,
        'train_bpi2013_t2025_10_7_0.003.pth',  # 模型文件路径
        device
    )

    print("\n前10个预测结果:", prelist[:10])
    print("前10个真实标签:", targetlist[:10])
    end_time = time()
